...
struct BinaryTree {
    ...
    TreeNode* find(int a) {
        ...
        while (!Q.empty()) {
            ...
            if (Q.front()->id_node == a) {return Q.front();}
            Q.pop();
        }
    }
    TreeNode* LCA(int a, int b) {
        ...
        for (int i = 0; i < 2 * nelements + 1; i++) {
            if (a == eulerian[1][i]) { ai = i; }
            if (b == eulerian[1][i]) { bi = i;}
        }
        if (ai > bi) {
            maxr = ai;
            minr = bi;
...
{
    BinaryTree Tree;
    Tree.insert(10);	Tree.insert(4);		Tree.insert(6);		Tree.insert(20);	
	Tree.insert(11);	Tree.insert(23);	Tree.insert(24);	Tree.insert(22);
    Tree.print();
    Tree.PreP1();
    Tree.print_eulerian();
    Tree.LCA(4, 6);
